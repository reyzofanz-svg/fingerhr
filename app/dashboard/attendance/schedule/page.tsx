  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-on-surface">Shift &amp; Jadwal Kerja</h1>
          <p className="mt-1 text-sm text-on-surface-variant">Buat shift, susun jadwal mingguan, lalu tugaskan ke karyawan</p>
        </div>
      </div>
        ...
        {([
          ...
          <button
            ...
            className={`flex-1 rounded-full px-4 py-2 font-medium transition-colors sm:flex-none ${
              tab === key ? "bg-primary text-white" : "text-on-surface-variant hover:text-on-surface"
            }`}
          >
            {label}
          </button>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {loading ? (
              ...
              <Card variant="glass" className="col-span-full">
                <CardContent className="py-12 text-center text-sm text-on-surface-variant">Belum ada shift</CardContent>
              </Card>
            ) : (
              shifts.map((s) => (
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-on-surface-variant">{s.startTime} - {s.endTime}</p>
                      </div>
                    ...
                    <div className="mt-4 flex gap-2">
                      ...
                      <button onClick={() => { setSelectedShift(s); setShiftModal("delete"); }} className="rounded-xl border border-error/20 bg-error/5 px-3 py-2 text-xs font-medium text-error transition-colors hover:bg-error/10">Hapus</button>
                    </div>
                  </CardContent>
                </Card>
              ))
      ...
      {tab === "jadwal" && (
        <div className="space-y-4">
          ...
          {shifts.length === 0 && (
            <Card variant="glass"><CardContent className="py-4 text-center text-sm text-on-surface-variant">Buat shift dulu sebelum menyusun jadwal.</CardContent></Card>
          )}
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            {workSchedules.length === 0 ? (
              <Card variant="glass" className="col-span-full">
                <CardContent className="py-12 text-center text-sm text-on-surface-variant">Belum ada jadwal</CardContent>
              </Card>
            ) : (
              workSchedules.map((ws) => (
                <Card key={ws.id} variant="glass-high">
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between">
                      <h3 className="text-lg font-semibold text-on-surface">{ws.name}</h3>
                    ...
                    <div className="mt-4 grid grid-cols-7 gap-1 text-center text-xs">
                      {ws.days.slice().sort((a, b) => a.dayOfWeek - b.dayOfWeek).map((d) => (
                        <div key={d.dayOfWeek} className="rounded-lg border border-white/[0.06] p-2">
                          <div className="font-medium text-on-surface-variant">{DAY_NAMES[d.dayOfWeek].slice(0, 3)}</div>
                          <div className={`mt-1 ${d.isDayOff ? "text-error" : "text-primary"}`}>
                    ...
                    <div className="mt-4 flex gap-2">
                      ...
                      <button onClick={() => { setSelectedJadwal(ws); setJadwalModal("delete"); }} className="rounded-xl border border-error/20 bg-error/5 px-3 py-2 text-xs font-medium text-error transition-colors hover:bg-error/10">Hapus</button>
                    </div>
                  </CardContent>
                </Card>
              ))
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-white/[0.08]">
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-on-surface-variant">Jadwal</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-on-surface-variant">Berlaku Dari</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-on-surface-variant">Sampai</th>
                      <th className="px-4 py-3"></th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/[0.05]">
                    {assignments.length === 0 ? (
                      <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-on-surface-variant">Belum ada penugasan</td></tr>
                    ) : assignments.map((a) => (
                      <tr key={a.id} className="transition-colors hover:bg-surface-container/50">
                        <td className="px-4 py-3 text-sm text-on-surface">{a.employee.name}</td>
                        <td className="px-4 py-3 font-mono text-sm text-on-surface">{a.employee.pin}</td>
                        <td className="px-4 py-3 text-sm text-on-surface">{a.workSchedule.name}</td>
                        <td className="px-4 py-3 text-sm text-on-surface">{new Date(a.effectiveFrom).toLocaleDateString("id-ID")}</td>
                        <td className="px-4 py-3 text-sm text-on-surface">{a.effectiveTo ? new Date(a.effectiveTo).toLocaleDateString("id-ID") : "-"}</td>
      ...
      {(shiftModal === "add" || shiftModal === "edit") && (
        <Modal title={shiftModal === "add" ? "Tambah Shift" : "Edit Shift"} onClose={() => setShiftModal(null)} wide>
          <div className="space-y-4">
            ...
            <p className="text-xs font-medium text-on-surface-variant">Batas jendela scan (opsional)</p>
            <div className="grid grid-cols-2 gap-4">
            ...
            <p className="text-xs font-medium text-on-surface-variant">Istirahat &amp; Lembur (opsional)</p>
            <div className="grid grid-cols-2 gap-4">
      ...
      {shiftModal === "delete" && selectedShift && (
        <Modal title="Hapus Shift" onClose={() => setShiftModal(null)}>
          <p className="text-sm text-on-surface-variant">Yakin ingin menghapus shift <strong>{selectedShift.name}</strong>?</p>
          <div className="mt-6 flex justify-end gap-3">
            <Button variant="secondary" onClick={() => setShiftModal(null)}>Batal</Button>
        ...
        <Modal title={jadwalModal === "add" ? "Tambah Jadwal" : "Edit Jadwal"} onClose={() => setJadwalModal(null)} wide>
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <p className="text-xs font-medium text-on-surface-variant">Atur tiap hari — centang Libur atau pilih shift</p>
              {jadwalForm.days.map((d) => (
                <div key={d.dayOfWeek} className="flex items-center gap-3 rounded-xl border border-white/[0.06] p-2">
                  <span className="w-16 text-sm text-on-surface">{DAY_NAMES[d.dayOfWeek]}</span>
      ...
      {jadwalModal === "delete" && selectedJadwal && (
        <Modal title="Hapus Jadwal" onClose={() => setJadwalModal(null)}>
          <p className="text-sm text-on-surface-variant">Yakin ingin menghapus jadwal <strong>{selectedJadwal.name}</strong>?</p>
          <div className="mt-6 flex justify-end gap-3">
            <Button variant="secondary" onClick={() => setJadwalModal(null)}>Batal</Button>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className={`glass max-h-[90vh] w-full overflow-y-auto rounded-3xl border border-white/[0.08] p-6 ${wide ? "max-w-lg" : "max-w-md"}`}>
        <h3 className="mb-6 text-lg font-semibold text-on-surface">{title}</h3>
        {children}
      </div>
    </div>
  );
