  ...
  return (
    <div className="min-h-screen bg-[#08080c]">
          ...
          <div className="relative flex-1">
            ...
            {cameraError && (
              <div className="absolute inset-0 flex items-center justify-center bg-black p-8">
                <div className="text-center">
                  <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-white/[0.08]">
          ...
          <div className="bg-black/90 backdrop-blur-sm p-6">
            <div className="flex items-center justify-center gap-8">
              ...
              <button
                ...
                className="relative flex h-20 w-20 items-center justify-center rounded-full bg-white shadow-2xl transition-transform active:scale-95 disabled:opacity-40 disabled:scale-100"
              >
                {isCapturing ? (
      ...
      <div className="p-4 pb-4">
        ...
        {result && (
          <div
            className="mb-6 rounded-xl p-4 border border-white/[0.12]"
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className={`text-sm font-medium ${result.success ? "text-white" : "text-white/80"}`}>
                  {result.message}
                </p>
        ...
        {/* Company Info */}
        <div className="mb-4 rounded-xl p-3 border border-white/[0.06]" style={{ background: "rgba(14, 14, 18, 0.7)", backdropFilter: "blur(12px)" }}>
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-white">PT. ANUGERAH CITRA TEKNOLO...</p>
              <div className="flex items-center gap-1 mt-1">
                <span className="relative flex h-1.5 w-1.5">
                  <span className={`absolute inline-flex h-full w-full rounded-full opacity-40 ${isInSpot ? "bg-white animate-ping" : ""}`}></span>
                  <span className={`relative inline-flex h-1.5 w-1.5 rounded-full ${isInSpot ? "bg-white" : "bg-white/30"}`}></span>
                </span>
        ...
        {/* Map Section */}
        <div className="mb-4">
          <LocationMap userLocation={location} spots={spots} isInSpot={isInSpot} />
        </div>

        {/* Location Status Card */}
        <div className="mb-6 rounded-xl p-4 border border-white/[0.06]" style={{ background: "rgba(14, 14, 18, 0.7)", backdropFilter: "blur(12px)" }}>
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-sm font-medium text-white">Status Lokasi</h3>
            <div className="flex items-center gap-2">
              <span className="relative flex h-2 w-2">
                <span className={`absolute inline-flex h-full w-full rounded-full opacity-40 ${location ? (isInSpot ? "bg-white animate-ping" : "bg-white/60") : "bg-white/40 animate-ping"}`}></span>
                <span className={`relative inline-flex h-2 w-2 rounded-full ${location ? (isInSpot ? "bg-white" : "bg-white/60") : "bg-white/40"}`}></span>
              </span>
              <span className="text-xs text-white/40">
                {location ? (isInSpot ? "Area kerja" : "Luar area") : locationError || "Mencari lokasi..."}
              </span>
            </div>
          </div>
          ...
          {location && spots.length > 0 && (
            <div className="space-y-2">
              {spots.map((spot) => {
                ...
                return (
                  <div key={spot.id} className="flex items-center justify-between rounded-lg bg-white/[0.04] p-3">
                    <div className="flex items-center gap-3">
                      <span className="relative flex h-2 w-2">
                        <span className={`relative inline-flex h-2 w-2 rounded-full ${isInRange ? "bg-white" : "bg-white/30"}`}></span>
                      </span>
                      <div>
                        <p className="text-sm font-medium text-white">{spot.name}</p>
                        <p className="text-xs text-white/30">Radius: {spot.radius}m</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className={`text-sm font-medium ${isInRange ? "text-white" : "text-white/60"}`}>
                        {Math.round(distance)}m
                      </p>
                      <p className="text-xs text-white/30">{isInRange ? "Dalam area" : "Jauh"}</p>
                    </div>
                  </div>
                );
              })}
        ...
        <div className="mb-6 rounded-xl p-1 border border-white/[0.06]" style={{ background: "rgba(14, 14, 18, 0.7)", backdropFilter: "blur(12px)" }}>
          <div className="flex gap-1">
            <button
              onClick={() => setAttendanceType("IN")}
              className={`flex-1 rounded-lg py-3 text-sm font-medium transition-all ${attendanceType === "IN" ? "bg-white text-black" : "text-white/40"}`}
            >
              Absen Masuk
            </button>
            <button
              onClick={() => setAttendanceType("OUT")}
              className={`flex-1 rounded-lg py-3 text-sm font-medium transition-all ${attendanceType === "OUT" ? "bg-white text-black" : "text-white/40"}`}
            >
              Absen Pulang
            </button>
          </div>
        ...
        {/* Photo Section */}
        <div className="mb-6 rounded-xl p-4 border border-white/[0.06]" style={{ background: "rgba(14, 14, 18, 0.7)", backdropFilter: "blur(12px)" }}>
          <h3 className="mb-4 text-sm font-medium text-white">Dokumentasi Absensi</h3>
          <div className="grid grid-cols-2 gap-4">
            <button
              onClick={() => openCamera("face")}
              className={`group relative overflow-hidden rounded-xl transition-all ${facePhoto ? "ring-2 ring-white/40" : "ring-1 ring-white/[0.08]"}`}
            >
              {facePhoto ? (
                <div className="relative aspect-[3/4]">
                  ...
                  <div className="absolute bottom-2 left-2 rounded-md bg-white/10 backdrop-blur-sm px-2 py-1">
                    <svg className="h-3 w-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                    </svg>
                  </div>
                ...
                <div className="flex aspect-[3/4] flex-col items-center justify-center gap-3 bg-white/[0.04] group-active:bg-white/[0.06] transition-colors">
                  ...
                  <div className="text-center">
                    <p className="text-xs font-medium text-white/60">Identifikasi Wajah</p>
                    <p className="text-xs text-white/30">Wajib</p>
                  </div>
                </div>
              )}
            </button>
            ...
            <button
              ...
              className={`group relative overflow-hidden rounded-xl transition-all ${surroundingPhoto ? "ring-2 ring-white/40" : "ring-1 ring-white/[0.08]"}`}
            >
              {surroundingPhoto ? (
                <div className="relative aspect-[3/4]">
                ...
                <div className="flex aspect-[3/4] flex-col items-center justify-center gap-3 bg-white/[0.04] group-active:bg-white/[0.06] transition-colors">
                  ...
                  <div className="text-center">
                    <p className="text-xs font-medium text-white/60">Foto Sekitar</p>
                    <p className="text-xs text-white/30">Opsional</p>
                  </div>
                </div>
              )}
            </button>
        ...
        {/* Notes */}
        <div className="mb-6 rounded-xl p-4 border border-white/[0.06]" style={{ background: "rgba(14, 14, 18, 0.7)", backdropFilter: "blur(12px)" }}>
          <label className="mb-3 block text-sm font-medium text-white">Catatan Absensi</label>
          <textarea
            value={notes}
            onChange={(e) => setNotes(e.target.value)}
            placeholder="Tambahkan catatan (opsional)"
            rows={3}
            className="w-full rounded-lg border border-white/[0.08] bg-white/[0.04] px-4 py-3 text-sm text-white placeholder:text-white/30 focus:border-white/30 focus:outline-none focus:ring-2 focus:ring-white/10 resize-none"
          />
        </div>
      ...
      <div className="fixed bottom-20 left-0 right-0 z-40 px-4">
        <div className="mx-auto max-w-md">
          <button
            ...
            className="w-full rounded-2xl bg-white py-4 text-sm font-bold text-black transition-all disabled:opacity-40 disabled:active:scale-100 active:scale-[0.98] hover:shadow-[0_0_30px_rgba(255,255,255,0.15)]"
          >
            {isSubmitting ? (
              <div className="flex items-center justify-center gap-3">
                <div className="loader" style={{ width: "16px", height: "16px", borderTopColor: "black", borderColor: "rgba(0,0,0,0.1)" }} />
                Memproses absensi...
              </div>
            ) : (
